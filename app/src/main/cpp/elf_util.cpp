...
ElfImg::ElfImg(std::string_view base_name, void* base) : elf(base_name), base(base) {
    // pt-injector changed: no need to find base
    /*
    if (!findModuleBase()) {
        base = nullptr;
        return;
...
std::vector<ElfW(Addr)> ElfImg::LinearRangeLookup(std::string_view name) const {
    ...
    for (auto [i, end] = symtabs_.equal_range(name); i != end; ++i) {
        ...
        LOGD("found %s %p in %s in symtab by linear range lookup", name.data(), (void *) offset,
             elf.c_str());
    }
    return res;
}
...
ElfW(Addr) ElfImg::PrefixLookupFirst(std::string_view prefix) const {
    ...
    if (auto i = symtabs_.lower_bound(prefix); i != symtabs_.end() &&
        ...
        LOGD("found prefix %s of %s %p in %s in symtab by linear lookup", prefix.data(),
             i->first.data(), (void *) i->second->st_value, elf.c_str());
        return i->second->st_value;
    } else {
        return 0;
    }
...
ElfImg::getSymbOffset(std::string_view name, uint32_t gnu_hash, uint32_t elf_hash) const {
    if (auto offset = GnuLookup(name, gnu_hash); offset > 0) {
        LOGD("found %s %p in %s in dynsym by gnuhash", name.data(), (void *) offset, elf.c_str());
        return offset;
    } else if (offset = ElfLookup(name, elf_hash); offset > 0) {
        LOGD("found %s %p in %s in dynsym by elfhash", name.data(), (void *) offset, elf.c_str());
        return offset;
    } else if (offset = LinearLookup(name); offset > 0) {
        LOGD("found %s %p in %s in symtab by linear lookup", name.data(), (void *) offset,
             elf.c_str());
        return offset;
    } else {
        return 0;
...
bool ElfImg::findModuleBase() {
    ...
    LOGD("get module base %s: %lx", elf.c_str(), load_addr);
